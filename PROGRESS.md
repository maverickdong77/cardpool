...
- **AskUserQuestion proofread hook 過於 aggressive、卡循環**：5/27 凌晨撞到 — `.claude/hooks/check_question_plain.ps1` 用 LLM judge 對 AskUserQuestion option label / description 檢查白話 + 繁體 + 不混簡日、但對 OCR-like false positive 過敏（如 hook 把「閃法」誤判「闃法」、把「量級」誤判「量隻」、把正確繁體「暫停」誤判簡體「暂停」），卡 3-4 次都過不了。**修法**：撞到 hook 第 2 次反彈、直接放棄 AskUserQuestion、改用「文字列出選項、user 直接回 1/2/3」格式。**通則**：未來決策性問題、若 user OK 簡單回字串、優先用 plain text 不用 AskUserQuestion；hook 過嚴時不要硬繞、直接 fallback text mode。
- **task wrapper 報 exit 127 ≠ python process 真死**：5/27 凌晨反覆撞 5+ 次 — `./Python/bin/python.exe run_api.py` 用 `run_in_background:true` 啟、Claude harness task wrapper 在某時間點（可能 90 秒 / 2 分鐘 timeout 等）報 `Background command "Start backend" failed with exit code 127`、但 `netstat -ano | grep ":8000 .*LISTENING"` 顯示 uvicorn process 還在 listen、API 正常 200 OK。**判斷**：backend 是否真活 = curl port / netstat、不依 task notification status。CLAUDE.md Pitfall #38 / #108 是類似 case（API silent crash / HTA watcher 接管 OLD code）、本條補上 task wrapper exit 127 也是同 family。**通則**：long-running background process（uvicorn / scraper daemon）的 task notification「failed exit 127」幾乎都是 wrapper timeout、不是 process 死、verify 真實狀態用 netstat / curl。
- **英文 listing_title「Poke Ball #64」是 item 卡、不是球閃變體**：5/27 寫變體 parser 撞到 — 1999 Jungle 套組的 `Poké Ball` trainer item 卡（卡名就叫精靈球、編號 #64/64）、簡單 regex `(poke|monster)\s*ball` 會誤判為「球閃變體」。**修法**：球閃 regex 必須加 `+ reverse holo / mirror / rev` context（`(poke|monster|pok[eé])\s*ball\s*(?:rev|reverse|mirror|holo)` 或 `(?:rev|reverse)\s*(?:holo|foil)?\s*(poke|monster|pok[eé])\s*ball`）。**通則**：對「同名詞既是卡名又是變體標記」的英文 keyword、regex 不能單獨 match、要加上下文限定（`reverse holo` / `mirror` context）才算變體；可考慮 `_parse_variant_title.py` 的 self-test 把這 case 明確 cover。
- **state.variant 跨頁殘留**：5/27 寫變體前端撞到、類似 Pitfall #55 state.catId 殘留 — user 從 variant=pokeball 詳情頁跳走後、state.variant 還在 pokeball、新頁帶錯 variant filter。Root cause：navigate / popstate 用 `Object.assign(state, parseHash())`、若 URL 沒 variant key、parseHash params 沒 variant、Object.assign 不會清掉舊 state.variant。**修法**：parseHash 結尾顯式設 `params.variant = null when 'variant' not in URL`、navigate / popstate Object.assign 時會清。**通則**：任何 URL-driven state 欄位（filter / mode / tab 等）、parseHash 都該顯式設預設 null、不靠「key 不在 params 就不覆蓋」。新增 SPA state 欄位時加進 parseHash 預設值清單。
- **httpx http2=True 要額外安裝 h2 package**：5/27 寫 _backfill_variant_images.py 撞到 — `httpx.AsyncClient(http2=True)` 拋 `ImportError: 'h2' package is not installed. Make sure to install httpx using pip install httpx[http2]`。embedded Python 沒裝 h2、預設 http/1.1 也夠用。**修法**：移除 `http2=True` 即可、`AsyncClient(follow_redirects=True)` 也能跑 5-10 並行抓 og:image 不慢。**通則**：未來寫 httpx async 不要預設加 http2、除非 explicit 需要（SNKR / pokemon-card.com / eBay 都 http2 可協商、但 http/1.1 也通）。
...
- **pokemon-card.com 搜尋頁 inline dropdown 只收近期 set、老期 promo 全沒收**：5/26 凌晨 Day 1 SM-P 起手撞到 — 搜尋頁 `<select id="expansionCodes">` inline 含 ~82 個近期 set（M-P / SV-P / pg=9XX 系列）、SM-P / S-P / XY-P / PCG-P / DP-P / E1-E5 等老期 promo + 老擴充包全沒收。**結論**：對 jp_card_list 未覆蓋的老 set、不能走 pokemon-card.com、必須走 SNKR title parse（SNKR mapping / snkr_hot_items 內 title 含完整 set_code + card_number + 卡名）+ pokellector / Bulbapedia 補圖。**通則**：未來補老期 set（S-P / XY-P / PCG-P / DP-P 等）一律 SNKR title parse 路線、不再嘗試 pokemon-card.com 搜尋。
- **寫進 jp_card_list 後必須同步寫 jp_card_pg_link**：5/26 凌晨撞到 — 寫 SM-P 396 張進 jp_card_list 後、`#/set?set=9005` set 頁仍「Set not found or empty」、空白頁。Root cause：`get_cards_by_set(set_id)` endpoint 用 `JOIN jp_card_pg_link l ON l.cardID = c.cardID WHERE l.pg = ?` 查、不直接從 `jp_card_list.pg` 查（N:N 設計、一張卡可屬多 pg）。jp_card_pg_link 表 pg=9005 0 條 → JOIN 不出來。修補：INSERT 進 jp_card_pg_link (cardID, pg) 396 條後 endpoint 才回 396 cards。**通則**：未來新爬蟲 / 補 jp set 卡時、必 INSERT 兩個表（jp_card_list + jp_card_pg_link）、缺一不可。CLAUDE.md「schema 雙寫一致性」原則升級版。
- **SNKR mapping 表 set_code/card_number 對「反序 [XXX SM-P]」格式不抓**：5/26 撞到 — snkrdunk_mapping.set_code / card_number 是 mapping parser 抽 SNKR title 結果、parser regex 對正序 `[SM-P 286]` work、但對「ムンク展」反序的 `[286 SM-P]` 不抓、set_code / card_number 都 NULL。要補抓必須從 snkr_hot_items 走 title regex（含正序 + 反序兩種）。實證 SM-P #286 コダック / #290 モクロー ムンク展兩條 mapping 全 NULL、補了之後 SNKR backfill 各拉到 140 / 335 筆 PSA 10 紀錄。**通則**：任何補 mapping 都先檢查 snkrdunk_mapping 該 set_code 的條目有沒 NULL row、必要時手動 UPDATE 補上 set_code + card_number、不然 sync_snkr lookup_apparel_id 找不到。
- **SNKR title 含「ポケモンカードゲーム教室」是教學配發普卡、不算 promo**：5/26 撞到 — SM-P snkrdunk_mapping 481 條中 37 條是教室普卡（C / U 稀有度、含 #009-#056 + #182-#190 基本能量教室版）、user 視覺檢查標記為「不該收進 promo set」。**修法**：parser EXCLUDE_PATTERNS 加 `'ポケモンカードゲーム教室'`、filter 掉。**通則**：未來補其他老期 promo set 時、同樣 filter 教室字串、避免普卡誤入 promo 卡單。
- **SNKR 商家偶爾上錯圖（apparel 166728 / 253253）**：5/26 撞到 — SM-P #091 第二張 (apparel 166728) title 標「ハイパーボール」但圖是阿羅拉椰蛋樹 / #108 (253253) title 標「ピカチュウ」但圖是阿羅拉雷丘。純 string parse 無法偵測、必須走 visual review HTML 人工抓。確認 CLAUDE.md「scraper 寫 DB 前 visual review」流程的不可替代意義。**通則**：未來補新 set 寫 DB 前必跑 visual review、預期商家錯標總有少數、blacklist 漏網 case。
- **frontend 同卡「外面 SNKR 商品掛賣價」vs「detail 頁 PSA 10 成交價」是兩個概念、user 容易混淆**：5/26 撞到 — user 點 set 517 #14 ライチュウ R、列表顯 ¥2,500（SNKR 掛賣、不分 grade、可能 RAW / 任何狀態）、detail 頁顯「尚無 PSA 10 成交紀錄」、user 困惑「為什麼外面有內面沒」。**修法**：`/api/prices/{set_id}/{cn}` response 加 `snkr_listing` 欄（fallback：查 snkr_hot_items 該卡的 listing）、frontend renderDetail 偵測 PSA 10 0 筆時 render 黃底 fallback block「⚠ SNKR 上仍有商品掛賣 NT$X / 賣家標價、不分 PSA 等級」+ 「在 SNKR 看商品 →」連結。**通則**：detail 頁設計要區分「成交紀錄」vs「即時掛賣」兩個概念、且對「外面有內面沒」case 一定要在 UI 加 fallback 訊息、不能讓 user 看到空白 + 沒解釋。
- **JPY 雙顯 (NT$X (¥Y)) 對 user 視覺不友善、改純 NT$X**：5/26 撞到 — 加 `fmtJpyDual` 顯「NT$550 (¥2,500)」user 反饋「日幣不要加進去 有點難看」、確認台灣 user 看純 TWD 比雙顯舒服。**修法**：fmtJpyDual 簡化只回 'NT$X'、不附 (¥Y)；footer 加匯率提示「JPY 1 = TWD 0.22 / USD 1 = TWD 32（寫死、非即時）」讓需要的 user 自己對照。**通則**：對台灣 user 一律只顯 TWD、JPY / USD 原值只在「明確需要原始貨幣」場景（如「SNKR 設定通知」alert modal）保留 ¥。
- **AskUserQuestion 選項 ≠ 動手批准、user 兩次糾正**：5/26 午後撞到 — 我問 jp_set_era_map 漏 M5 怎處理、列 3 個選項給 user、user 選「只修 M5」option（description 寫明確 SQL）後我直接 INSERT、user 反彈「我說過改動前你都要問我」。後來再選 DELETE 退訂時也是要 user explicit「跑 DELETE」+ 我把 SQL 完整貼出來才能跑。**修法**：流程改兩段 — (1) AskUserQuestion 拿方向 → (2) 顯示完整最終指令（SQL / Edit diff / 將動的檔）+ 拿 explicit「go」訊號 → (3) 才執行。已升級進 `feedback_ask_before_decisions.md` memory。**通則**：任何寫 DB / 改檔案 / 動 schema / 跑 backfill 等留痕動作、option 選項只算「方向確認」、執行前要再 final confirm 一次。
- **HTA / watcher 接管 OLD code PID — 改 backend 後可能 watcher restart 但跑舊 code**：5/26 午後撞到 — 改 `app/main.py` 加 cdn.snkrdunk.com 到 proxy_img 白名單後、kill listening PID + Start-Process 新啟、但 30 秒後 netstat 抓到的 PID 啟動時間早於 main.py mtime → 跑舊 code、proxy 全 403。`Get-CimInstance Win32_Process -Filter "ProcessId=N"` 看 CreationDate 比 main.py LastWriteTime 早就是舊 code。**修法**：kill PID + Start-Process 新啟、`Start-Sleep -Seconds 5` 等 boot 完、curl 驗 endpoint 真的回新行為（不只看 200）。CLAUDE.md Pitfall #42 已記、本次再驗證、加強：**verify behavior 不只 verify status code**。
- **CSS 純 object-fit 沒辦法處理「圖內含 padding 且 padding 量變異」的多源資料**：5/26 午後撞到 — 今日熱門頁有 jp_card_list（純卡圖 0.716）+ SNKR cdn（卡置中、padded 在 1.368 canvas、padding 量每張不同）兩種源。用 cover 讓 frame 一致但 SNKR cdn 卡內容仍偏小、用 contain + 縮 jp_card_list 太小、scale 統一倍數會切到某些卡。**真正解**：走 D 方向 — 透過 `/api/proxy_img` 同源 fetch 圖、canvas 量出每張 SNKR cdn 圖實際 card bbox、套 `transform: scale(1/heightPct)` 動態算每張的放大倍數、結果 cache 在 localStorage。實證 user 驗收「有了 可以」。**通則**：未來碰到「多源資料 + 圖內容 padding 變異」case、純 CSS 沒解、要不就 backfill 統一源、要不就 D 方向 canvas 動態量。
- **「視覺風格」需求迭代 8+ 輪仍不對才換思路、CLAUDE.md「超過 3 輪」原則本次驗證**：5/26 午後縮圖一致性問題反覆 8 輪（cover / shrink / scale 1.35 / scale 0.55 / 各種倍數）、user 提示「用 brainstorming 解決這問題」+「列出問題然後跟我討論」才換 D 方向。CLAUDE.md feedback「超過 3 輪不對主動 stop 改方向」本次再驗證、實作流程是要主動換思路、不要等 user 推。
- **PIL `Image.size` 報的是 webp 畫布尺寸、不是內容尺寸**：5/26 晚撞到 — SNKR `upload_bg_removed/` webp 報 1000×730（橫式比例 1.37）、但實際卡牌內容是直式（左右大量透明 padding）、用 `getbbox()` 找非透明 pixel bounding box 才看到真內容 429×610（直式 0.703）。**通則**：對 RGBA webp / PNG 帶大量透明 padding 的圖、不能只看 `Image.size` 推 ratio、要用 `getbbox()` 算實際內容 bbox。本次 cropped_snkr_img endpoint 用此邏輯 crop。
- **endpoint Cache-Control 設長 max-age 撞 browser cache、邏輯改後 user 看不到新版**：5/26 晚撞到 — cropped_snkr_img endpoint v1 設 `Cache-Control: public, max-age=86400`（1 天）、邏輯改成「升級 xl 原圖」後 user reload page 仍拿到舊 medium 版本。**修法**：(a) max-age 改 300 + must-revalidate + ETag、(b) wrap URL 加 cache buster query（如 `&v=hd1`）、邏輯改時 bump 版本字串、瀏覽器視為新 URL 必重抓。**通則**：dev 階段任何「邏輯可能改動」的 endpoint 都用短 cache + ETag、或預設加 cache buster query 給 caller 帶；avoid 設長 cache 後 user 拿不到新行為。
- **CSS animation + JS `set transform` 同 element 撞、互蓋**：5/26 晚撞到 — T3/T4 想加上下飄動（animation: `translateY`）但 `.card-tilt` 已被 JS mousemove set `transform: rotateY/X`、兩個 animation 都改 transform 屬性會互蓋。**修法**：加 wrapper element `.card-bob` wrap `.card-tilt`、外層套 float animation、內層 `.card-tilt` 保持 JS 3D transform、兩 layer 分職。**通則**：CSS animation 跟 JS-driven transform 共存時、必須分 element 套用、同一 element 不能兩個系統都動 transform。
- **multi-layer overlay 系列 element z-index 要 enforce 一致、漏改一個就被遮**：5/26 晚撞到 — mockup v3 升級 sparkle 配方時、`.fx-10 .sparkle-layer` z-index 改 7 ✓、但 `.fx-2 .sparkle-layer` 漏改、仍 z-index 4 < `.card-tilt` z-index 5、sparkle 被卡擋住、user 看到「跑到底下」反饋。**修法**：所有 overlay 系列 layer（sparkle-layer / holo-under / corner-glow 等）統一 z-index ≥ 6（card layer + 1）。**通則**：CSS 多 layer 系統先寫一個 layer constant 表（z-index: 0 背景 / 5 card / 6+ overlay）、別逐 layer 各設不同數字。
- **drop-shadow 多層太重會糊掉小元素本體**：5/26 晚撞到 — mockup sparkle 用 7px 圓 + 3 層 drop-shadow (6/14/22 px)、user 評「只看到金色光、看不到火花顆粒」。Root cause：3 層 drop-shadow 暈光範圍超過 50 px、淹沒中心 7px 顆粒 + 28px 偽元素 spike。**修法**：用 SVG 4 角星形 path（漸窄 spike + clear stroke 輪廓）+ 1 層淡 drop-shadow（5 px）、星形本體輪廓清晰、不被 glow 蓋掉。**通則**：對「小發光元素」（sparkle / pixel star / 火花等）、用 SVG path shape 比 div + 多層 drop-shadow 更穩；drop-shadow 1 層當 hint glow 就好、不堆疊。
- **MCP playwright chrome residue 卡住 `browser_navigate` + 殺 root PID 不夠**：5/26 晚撞到 — `browser_navigate` 報「Browser is already in use for mcp-chrome-c7f8c88」、要 PowerShell `Get-CimInstance Win32_Process -Filter "Name='chrome.exe'"` `Where-Object CommandLine -like "*mcp-chrome*"` 找出 7 個 process（root + 6 個 sub-process: gpu-process / utility / renderer 等）、全部 `Stop-Process -Force` 殺乾淨再 navigate 才能 OK。**通則**：MCP playwright chrome 殘留要殺、不能只 stop root PID、要找所有 user-data-dir = `mcp-chrome-*` 的 process 一併殺。CLAUDE.md `force_playwright_for_user_urls` 規則背後配套。
- **system task notification 完成 ≠ background process 死、要 netstat verify**：5/26 晚撞到 — `Start backend in background` task wrapper 報 `exit code 0 completed`、我以為 backend uvicorn process 也死了、報「backend 自己 exit、要重啟」、實際 `netstat -ano | grep ":8000.*LISTENING"` 顯示 PID 33836 仍 listening、API 正常服務。task wrapper 結束 ≠ 它啟動的 daemon process 結束。**通則**：判斷 backend process 是否真活要 query port LISTENING 狀態 / curl endpoint、不憑 task notification 完成 status。CLAUDE.md Pitfall #38（API silent crash）逆向版。
- **`AskUserQuestion` option 文字反覆出現錯字 / 詞義不清、memory 規則已寫多次仍犯**：5/26 晚 session 內違規 2+ 次（「複製品」「實千寥隻」等詞義不清的中文）、user 明確抓包「字又錯了」「先想辦法修正這問題」。memory `feedback_askuser_proofread` 規則 + CLAUDE.md「白話 + 括號附原文」+「白話 + 字念過」原則都寫了、但 implementation 階段仍憑語感打字、沒落實「每字念過」。**修法**：寫每個 question / option label / description 前、心裡先用「整句白話講過」再凝縮成 option 文字、然後逐字念過抓 4 種錯（打錯字 / 詞義不清 / 混簡 / 混日）、不通就重寫。**通則**：本規則為「執行紀律 + 已知會反覆違規」級別、未來 session 開始時主動 prime 自己「寫 option 前念過」。已存進 [askuser-proofread memory](memory/feedback_askuser_proofread.md) 強化。
- **「黑名單 = 永久刪除」假設錯誤、user 工作流程是「黑名單 from S-P 主集合 + 搬到專屬收容 set」**：5/27 撞到 — 處理 S-P 跨期混入 #087 #088 #089 #166 #167 #277 等卡時、我看到 user 把它們加進 `_gen_sp_review.py` `BLACKLIST_APPAREL`、就假設「user 想徹底刪除這些卡」。對第三輪 #256 / #344-350 又 8 張跨期重印卡、我繼續用「加進 BLACKLIST 就好」的方案 propose 給 user。**user 反彈「為什麼要加入黑名單」「不要刪掉 請列表出來給我」**、我才回頭查、發現資料庫**早已存在 `jp_card_list_set` pg=9007 set_code='S-P-CROSS' 「ソード&シールド SNKR 跨期混入特典」、且已收 6 張**（就是上述 #087~#277）。user 的真實工作流程是：(1) 從 S-P 主集合（pg=9006）BLACKLIST 移除、(2) 另寫進 pg=9007 收容 set、保留卡片資料 + 維持 SNKR 查價能力。Root cause：我看到 BLACKLIST 增加時沒問「這些被剃掉的卡會去哪」、單方面用最保守的「永久刪」解讀。**修法**：未來看 user 修改 BLACKLIST / EXCLUDE / 跳過清單時、必問「被剃出來的卡會去哪」、grep 資料庫看有沒對應的「收容 set / 替代 set / overflow set」存在。SQL 查 `WHERE name_jp LIKE '%跨期%' OR name_jp LIKE '%混入%' OR name_jp LIKE '%cross%' OR name_jp LIKE '%overflow%'` 看有沒 placeholder set。**通則**：對「批次過濾 / 黑名單」型決策、不要假設「被過濾的就是不要的」、要問「被過濾的去哪、走什麼分支」。Black/white list 的對立面常不是「刪除」、而是「分支到別的 bucket」。違反代價：user 看到我準備「永久刪 13 張卡」、明確警告「不要再犯、卡片被誤刪不是很好」。
...
### 2026-05-25（再延伸 — 收工 GitHub repo + 交接工程師準備）

純整理 + 交接準備 session、code 0 行改動、共 7 個 commit 推進歷史 + 1 個 Private GitHub repo push 上線 + 1 份 local-only 交接清單。

#### 完成

**1. CLAUDE.md 清掉 4 處過時歷史紀錄（commit ea9b4c3）**
- 拿掉 `card_prices` 描述後半「2026-05-18 Phase 4 後 132k junk → 10,407 清版」歷史數字
- 拿掉 `snkrdunk_mapping` 描述「is_pokemon flag 歷史誤標多（已 sweep 修補 3,604 rows）」、改寫成現況
- 整段刪「jp_term_dict 建構流程（2026-05-17 建立）」6 行 sub bullets（字典已建好、未來不會再跑那個一次性 pipeline）
- 「JP eBay 覆蓋預期（Phase 4 後）」→「JP eBay 覆蓋現況」
- 用「backup mixed state → checkout HEAD → 重做改動 → commit → 還原 backup」拆 patch 法、只 commit 我這 session 改的 4 處、保留 user 自己累積的其他 CLAUDE.md modifications 在 working copy

**2. 5 個 modified file 拆 5 個語意 commit**
- `c561dab` chore(gitignore): 加 ONBOARDING.md 排除規則（Claude Code internal、不該進 repo）
- `fb9e20c` docs(progress+claude): 5/24-5/25 累積工作日誌 + Known Pitfalls + 偏好升級（+387/-4）
- `6f066c1` docs(plan): 加 set-code 三表對映 backfill plan（+398、含 backup / cascade dedupe 順序 / endpoint dedupe key 切換）
- `70f40f5` feat(db+marketplace): 盒裝交易撮合模組 + 到價通知 schema（+121/-4）
  - `app/database.py` 加 `price_alerts` + `notifications` 兩表 + 4 個 index
  - `app/marketplace.py` 新檔：Phase 2 Bid/Ask 訂單簿撮合骨架（含盒裝偽 ID `set_id='__box__'`）
- `7aef79f` feat(api): 5/24-5/25 累積多項 endpoint（+731/-135）
  - 盒裝交易 7 個 endpoint（alerts CRUD + notifications + orderbook depth）
  - SNKR 熱門首頁爬蟲 + LINE 推送 helper + 撮合觸發 check
  - character/pokemon endpoint 改三表 UNION + (set_code, card_number) dedupe
  - proxy_img endpoint（跨域 Canvas 走 same-origin proxy、白名單 artofpkm/pokemondb/PokeAPI）

**3. 清 66 個 untracked debug 檔**
- 60 個 PNG（過程截圖、無底線開頭、`.gitignore` 沒 cover）
- 5 個 `_spike_*.xml` + 1 個 `.xml.gz`（sitemap 探勘暫存）
- 用 `git ls-files --others --exclude-standard | grep` 精準過濾、避免誤刪非 debug 檔
- 不用 `git clean -f`（太危險、會刪所有 untracked）

**4. README.md 重寫（commit 3e181cd、+194/-123）**
- 舊 README 描述 `python -m app.main` / `static/index.html` 都已過時、工程師按舊 README 跑會踩雷
- 新 README 含 9 章：雙伺服器架構圖 + 本機開發環境 + 啟動指令（手動 / HTA / 重啟）+ Windows cp950 注意 + 後端目錄結構 + 11 張 DB 表簡介 + 5 類 API endpoint 分類 + eBay/SNKR 爬蟲設計重點 + 翻譯管線 7 步驟摘要 + 給接手工程師的閱讀順序 + 部署設定
- 明確警告姊妹目錄 `..\卡波\` 跟 `cards.db` 不在 repo 內、要另外索取

**5. Private GitHub repo 建好 + 全歷史 push**
- repo URL：https://github.com/maverickdong77/cardpool（私人、只給 invited collaborator 看）
- user 在網頁 https://github.com/new 建（選 Private、不勾 README/.gitignore/license 三項避免初次 push conflict）、給我 URL
- `git remote add origin https://github.com/maverickdong77/cardpool.git`
- `git push -u origin master` 一次過、整個 commit 歷史推上去

**6. 寫 `_handoff_checklist.md`（local-only、`.gitignore` 的 `_*.md` 規則自動排除）**
- 6 章：要交給工程師的三樣東西（GitHub invite + 卡波.zip + cards.db）/ 起專案步驟（含解壓位置警告）/ 閱讀順序 / 重要踩坑提醒 8 條 / smoke test 4 點 + fail 對應診斷 / owner 自查 checklist 5 條（敏感檔案 / 真實用戶資料 / 權限約定）

**7. 準備兩個附件 → 後來 user 改方向都刪掉**
- 壓縮 `..\卡波\` → `Desktop\卡波.zip`（343 files / 3.1 MB → 1.5 MB、`Compress-Archive -CompressionLevel Optimal`）
- copy `cards.db` → `Desktop\cards.db`（814.7 MB、0.2 秒 SSD copy）
- user 改主意「不用了給他看架構就差不多」、兩個都從桌面刪、原本的 `cards.db`（backend 用的）安全保留

**8. 整理精簡版交接訊息**給 user 直接 copy
- 不含 cards.db / 卡波.zip 段落
- 含 repo URL + 閱讀順序 + 技術 stack + user 想聽哪些方向的意見（後端模組化 / 爬蟲長期 anti-bot / 資料模型 / 盒裝交易 Phase 2 撮合）
...
- **Bash tool 跑 PowerShell 語法失敗**：寫 `if (!(Test-Path ...)) { ... }` 給 bash、`{` 被 bash 解析錯。解法：切 PowerShell tool 跑。**非結構性、純 tool 切換問題**。
- **`Edit` tool 對 CLAUDE.md 第一次撞「File has been modified since read」**：可能 lint hook 或平行 session 改動。解法：重 Read 後再 Edit OK。**非結構性、是 Edit tool 正常 behavior**。
- **以為「5 處可清」、實際只有 4 處能單獨清**：① 那條「README.md 提到的 static/index.html 已刪除... — 以本檔為準」是「disclaim 過時 README」、user 選不改 README、所以 ① 不能單獨刪、要保留。提醒：清理 reference 性內容前、要確認 reference 對象（README）是否同步改、否則只刪 reference 會留下「README 仍過時但沒人 disclaim」的真空。

#### 明天的下一步

1. **等工程師回饋**：user 邀請工程師到 https://github.com/maverickdong77/cardpool 看 architecture、聽他對「後端模組化（單檔 main.py ~85 endpoint）/ 爬蟲長期 anti-bot 對策 / 資料模型多語言三套並存 / 盒裝交易 Phase 2 Bid/Ask 撮合」的意見
2. **延續 5/25 傍晚〜深夜未動方向**：
   - B2 fetch 沒帶 API_BASE（line 2771 1 行改）
   - B4 hero 數字 lang-reactive 但「成交紀錄」不變
   - B5 portfolio demo 跨遊戲卡（要決定 placeholder vs 真接通）
   - B6 EN「Other」分組過大（要看 era_map 是否漏對映）
   - 重生 `docs/jp_sets_lookup.md` + 考慮加 `docs/tw_sets_lookup.md`
   - TW 32 新 set 卡片資料補抓（card_count=0）
3. **延續 5/24 凌晨延伸未動方向**（仍未動）：71 張高稀有度 0-row eBay verify / Portfolio Phase 2 後端 API / MVP S1 auth KYC / GoldenGem Phase A 自選頁 / Plan Task 5-22 重寫 jp_set_backfill / XY-P #75 修圖 / card_prices jp- vs pg 雙系統合併
4. **盒裝交易 Phase C**（看 user 意願 + 工程師意見）：ECPay 沙盒 / KYC / 撥款流程
5. **LINE Bot 綁定 flow + 自動排程**（看 user 意願）

### 2026-05-26（凌晨）— Day 1 SM-P 起手：補 jp_card_list 沒收的老期 promo set（396 張寫進、13,059 筆 SNKR PSA 10）

接 5/25 plan「補 3,864 件未對映 SNKR 熱門卡」、user 選「全部補、老期 set 一起爬」。Day 1 起手 SM-P 太陽月亮 promo（snkrdunk_mapping 481 條 / SNKR 熱門 181 件、jp_card_list 0 張）。

#### 完成

**1. 來源策略 pivot**
- 試 pokemon-card.com 搜尋頁、inline `<select id="expansionCodes">` dropdown 只收 82 個近期 set（M-P / SV-P / pg=9XX）、SM-P 完全沒收（已加進 Known Pitfalls）
- pivot：SNKR title 自帶 set_code + card_number + 卡名三件套、不需外部來源、直接 parse SNKR title

**2. parse + visual review HTML 8 輪迭代**
- `_smp_parse_spike.py`：parse 481 條、437 條成功（91%）
- `_fetch_smp_imgs.py`：對 264 條無 image_url 的、爬 SNKR `/apparels/{id}` og:image、全 437 件有圖
- `_gen_smp_review.py`：生 _smp_review.html 含 437 張 grid + checkbox + 匯出名單、`python -m http.server 8081` serve
- user 視覺檢查報 8 大類問題：古代ミュウ #2019 不在 SM-P / #091 #108 圖錯 / 教室普卡 #009-#056 + 基本能量 #182-#190 / #286 #290 漏 / 通路名混卡名 #171/#174/#177 / 活動名「争奪戦プロモ」#239/#240/#363/#364 / 半形括號 #381
- 修 parser：EXCLUDE_PATTERNS 加「ポケモンカードゲーム教室」+ BLACKLIST_APPAREL 4 條 (古代ミュウ + 圖錯 2 件) + `normalize_card_name`（砍 `*通路名` / `: 活動名プロモ` / 半形括號改全形）+ 反序 regex `[XXX SM-P]` 補抓 #286 #290
- 397 → 396 張乾淨寫進 jp_card_list

**3. 寫 DB（backup cards.db.before-smp-write-20260525-230056）**
- jp_card_list +396 張（pg=9005、set_code='SM-P'、thumb_url 用 SNKR cdn 圖）
- jp_card_list_set +1 條（hit_cnt=396、name='ポケモンカードゲーム サン＆ムーン プロモカード (寶可夢卡牌遊戲 太陽&月亮 特典卡)'、release_date=2016-12-09）
- snkr_hot_items UPDATE 170 件（set_id='9005' + card_number + image_url_local）
- 漏寫 jp_card_pg_link 導致 `#/set?set=9005` 「Set not found or empty」、補寫 396 條 link 才解（已加進 Known Pitfalls）
- 後補 #286 / #290 ムンク展（反序格式 + UPDATE mapping NULL row）、最終 398 張

**4. SNKR PSA 10 backfill（5 分鐘）**
- `_backfill_smp_snkr.py` 8 並行：396/396 ok / 0 fail / 12,859 筆
- 補 #286 / #290 各 140 / 335 筆、總 **13,059 筆**
- Top 5：#381 ピッピ（リーリエ）1,838 筆均 ¥87k / #270 788 筆 / #297 521 筆 / #407 498 筆 / #288 454 筆均 ¥861k

**5. eBay PSA 10 backfill（背景跑中）**
- `_backfill_smp_ebay.py` c=2 / 開背景、wrap 時進度 212/396 (53%) / 90 筆已寫入 / ETA 1-2 小時

**6. backend 改造**
- 3 處 SQL 加 thumb_url conditional：`CASE WHEN thumb_url LIKE 'http%' THEN thumb_url ELSE 'https://www.pokemon-card.com' || thumb_url END` — 支援絕對 URL + 相對路徑並存
- `/api/prices/{set_id}/{cn}` 加 `snkr_listing` 欄、回該卡 snkr_hot_items 內的 listing（apparel_id / price_jpy / image_url / url）

**7. frontend 多項改造（`..\卡波\index.html`）**
- `classifySnkrItem` 加「未開封パック」keyword：整包 promo pack 移到「特典卡包」桶
- snkr_hot 列表 click：未對映卡不外連 SNKR、改 `openSnkrFallbackModal` 顯本站 modal
- detail 頁加「SNKR 掛賣價」黃底 fallback block（PSA 10 0 筆時 render、含 NT$ + 連結）
- 加 `fmtJpyDual(jpy)` helper（JPY 0.22→TWD 換算）
- 雙顯 NT$X(¥Y) 簡化成純單顯 NT$X（user 反饋「日幣不要加進去 有點難看」）
- footer 加匯率提示「JPY 1 = TWD 0.22 / USD 1 = TWD 32（寫死、非即時）」
- 拿掉 snkr_hot 卡片左上角排行號碼徽章
- BOX 標籤改用前端 classifySnkrItem 結果（不用 DB is_box）
- 「↗ SNKR」改「無本站資料」黃底徽章

**8. set 517 ライチュウ #14 case 解決**
- user 報「外面有顯示價格但點進去沒有」、查發現是「SNKR 掛賣價」（即時 list price、不分 grade）vs「PSA 10 成交紀錄」兩個不同概念
- 解法：detail 頁 fallback block 覆蓋全站、所有「PSA 10 無紀錄但 SNKR 有掛賣」的卡都顯
- 確認 ライチュウ #14 真實 SNKR 無 PSA 10 成交歷史（不是 backfill 漏、是真實 0 筆）

#### 進行中 / 待做

- **eBay backfill**（background task `bm123ybyy`）：53% 進度、wrap 時繼續跑、user 等跑完才評估 Day 2
- **未 commit**：app/main.py / `..\卡波\index.html` / PROGRESS.md / cards.db 已改、+ 多個 _*.py local-only spike scripts

#### 踩到的坑（已加進上方 Known Pitfalls）

8 條新加：

1. pokemon-card.com 搜尋頁 inline dropdown 不收老期 promo set
2. 寫 jp_card_list 必須同步寫 jp_card_pg_link
3. SNKR mapping 表對「反序 [XXX SM-P]」格式 set_code NULL
4. ポケモンカードゲーム教室 是教學普卡、不算 promo、要 filter
5. SNKR 商家偶爾上錯圖、必跑 visual review 才抓得到
6. frontend price_twd（成交）vs price_jpy（掛賣）兩個概念、容易混
7. detail 頁要加「SNKR 掛賣價」fallback block 給「外面有內面沒」case
8. JPY 雙顯不友善、改純 NT$X

#### 明天的下一步

1. **看 eBay backfill 結果**：query DB 看 SM-P 396 張中 eBay PSA 10 覆蓋多少；高稀有度（SAR / SR / UR / AR / RR / HR / CHR / SSR / MUR / CSR）0-row 列表給 user verify
2. **Day 2 起手 set 候選**：
   - S-P 劍盾 promo（119 件、推薦、近期 set mapping 較齊全）
   - XY-P（98 件）
   - PCG-P（75 件）
   - DP-P（40 件）
3. **拆 commit**：本 session 累積 app/main.py / `..\卡波\index.html` / PROGRESS.md / cards.db 待 commit
4. **延續 5/25 未動方向**：B2 fetch API_BASE / B4 hero 數字 / B5 portfolio demo / B6 EN「Other」分組 / TW 32 新 set 卡片補抓 / 71 張高稀有度 0-row eBay verify / Portfolio Phase 2 / MVP S1 auth KYC / GoldenGem Phase A / card_prices jp- vs pg 合併

### 2026-05-26（深夜延伸）— PSA POP Dashboard 微調 + Bug B2/B4/B6 + EN retry 全量背景跑

接續同日凌晨 PSA POP 主體上線後、繼續 6 個追加任務：

#### 完成

**Task 17 — Dashboard 加「PSA 官網查看」連結**（commit 後端 `5035a34` + 卡波 `12d3ed7`）
- 後端 `get_card_prices` SQL LEFT JOIN `psa_apr_card_mapping`、response 多回 `psa_spec_id`
- 前端 Dashboard 加「在 PSA 官網查看 ↗」按鈕、連到 `https://www.psacard.com/spec/psa/{spec_id}?p=population`
- 解 user 訴求「我要點過去 PSA 主頁對照才能 verify 數字」

**Task 18 — 修 17 張 spec 對映錯誤**（純資料層、無 code commit）
- user 截圖證實 `jp-10th-Movie-Commemoration-Set` 整 set 11 張對映到 PSA 2004 SKY'S RAYQUAZA、實際我們是 2000 結晶塔エンテイ
- 寫 `_sanity_check_jp_pop.py` 對 30 張 jp- has-POP 跑 spec page 卡名 token 比對、找出第 2 批 6 張 mismatch（jp-White-Flare 對 2003 EX Crystal Guardians）
- 17 張 SET NULL psa_pop_* + DELETE 17 條錯 mapping

**Task 19 — Dashboard label 改白話 + footer 高亮**（commit 卡波 `594dbe3`）
- 「PSA 10 通過率」→「拿到 PSA 10 滿分的比例」（圓圈旁）
- 「總鑑定數 / GEM 通過率」→「PSA 累計鑑定 N 張 / 拿到 PSA 10 滿分 N%」（footer）
- footer 樣式：灰底 → 淺黃底 + 邊框、數字 13px → 16px 加粗

**Task 20 — Bug B2 / B4 / B6**（commit 卡波 `4899b1d` + DB 直接 UPDATE）
- B2：`refineJpCardBackByDate()` fetch 加 `${API_BASE}` 前綴（line 3301）
- B4：首頁 hero 「成交紀錄」label 加 i18n、隨 setLang 切 EN/JP/TW
- B6：15 個 EN「Other」分組 set 補 era_map、直接 UPDATE `en_card_list_set.series`、結果 Other = 0

**Task 21 — EN retry quick test 100 張**（無 code commit、純驗證）
- 第一輪 LIMIT 100 全卡在 en-Ascended-Heroes（unmatched.txt 字串排序、set 209 張連續）、全 MISS
- 自審：早期判斷「PSA IP cooldown」是錯的、root cause 是 stdout block-buffered + sample 全 master-numbering set

**Task 22 — EN retry v2 多 set 打散 sample**（純驗證）
- 從 top 10 EN unmatched set 各取 10 張、總 100 張
- **最終 hit=59 / miss=36 / hit_rate=62.1%**
- 5 個 set 100% 可救：Paldean-Fates / Paradox-Rift / Paldea-Evolved / Twilight-Masquerade / Scarlet-Violet-English
- 2 set 完全不可救：Ascended-Heroes（master numbering）/ Destined-Rivals（待查）

**Task 23 — EN unmatched 全量 retry（已完成、05-27 ~00:50）**
- `_retry_en_full.py` 跑 **1818** EN unmatched 卡（實際範圍小於估的 ~2,050、扣已 sample hit + 已 has-POP 卡）
- **最終 hit=1343 / miss=475 / hit_rate=73.9%**（比 Task 22 sample 估的 62% 高 12pp）
- `has-POP` total: 2,137 → **3,480**（+1,343）
- EN has-POP: 309 → **1,652**（EN 覆蓋率 8.3% / 19,874 EN 卡）
- JP has-POP: 1,828（沒動）
- **4 整 set 0% hit**（PSA 不收）：Ascended-Heroes 0/88、Destined-Rivals 0/49、Perfect-Order 0/42、Scarlet-Violet-151 0/48
- **低 hit set**（< 50%）：Black-Bolt-EN 1/30 = 3.3%、Mega-Evolution 1/7 = 14.3%、Journey-Together 10/43 = 23.3%、White-Flare-EN 34/80 = 42.5%、Phantasmal-Flames 21/41 = 51.2%
- **30+ set 100% hit**：Lost-Thunder / Ultra-Prism / Stormfront / Sun-Moon / Forbidden-Light / Vivid-Voltage / Guardians-Rising 等老 set 全可救

#### 進行中

無 — EN retry 全量已完、Task 23 final 結果已寫進本段上方。

#### 踩到的坑（已加 Known Pitfalls）

新加 3 條：

1. **Python stdout redirect to file 是 block-buffered**：要 `python -u` 或 `print(..., flush=True)` 才能即時看 log；7 分鐘 log 0 bytes ≠ process 卡、可能是 buffer 沒滿、不要 panic kill
2. **PSA APR EN 系列分兩類**：(a) 用 Pokémon-only 序號（Paldean-Fates / Paradox-Rift 等可救、search query 不加 Japanese keyword 即可）/ (b) 用 master numbering（Ascended-Heroes 不可救、card_number filter 100% 對不上）
3. **unmatched.txt 字串排序撞「單一 set 連續 200+ 張」**：sample test 要按 set 分組打散、否則前 100 全同 set 結果無代表性

#### 明天的下一步

1. EN retry 低 hit / 0 hit set 後續調查：4 個 0% set（Ascended-Heroes / Destined-Rivals / Perfect-Order / Scarlet-Violet-151）是「PSA 還沒收」還是 search query 對命名規律對不上？需 spot check 1-2 卡實際 PSA 網頁
2. 高稀有度 0-row eBay verify（PROGRESS.md 延續任務、5/22 列 71 張）
3. 整理 `_*.py` spike scripts（全 local-only `.gitignore` 排除、選擇局部留 / 局部刪）
4. SM-P eBay backfill 驗收 / Portfolio Phase 2 真接通 / MVP S1 auth KYC / card_prices jp- vs pg 雙系統合併（~1-2 hr）

---

### 2026-05-26（凌晨延伸）— PSA POP 真實鑑定存世量整合 + 該不該送鑑定推薦器

User 給 2 篇參考資料（samuraiswordtokyo / pikawu）後、設計 + 實作把詳情頁舊「PSA 拍賣分布」升級為真 PSA POP + 推薦器。完整 brainstorming → spec → plan → subagent-driven 流程跑完前 11 個 task。

#### 完成

**1. Spike + spec + plan（commit `b05d2a5` / `8d78921`）**
- spike 驗證 PSA 官方 `populationSummary` endpoint：`GET /api/psa/researchJourney/spec/{spec_id}/PSA/populationSummary?filter=all`、過 Cloudflare 後 200 OK
- 兩張卡實測：Magikarp #80 三重節拍 真 POP=59,120 / PSA 10=53,912 / GEM=91.19%（vs 舊「拍賣筆數」6,043、差 9 倍）；Eevee #78 朱紅迷霧 真 POP=31,310 / GEM=86.23%
- spec：`docs/superpowers/specs/2026-05-25-psa-pop-grading-recommender-design.md`、用 brainstorming skill 4 塊逐段同意
- plan：`docs/superpowers/plans/2026-05-26-psa-pop-grading-recommender.md`、16 task / 5 phase

**2. 資料層（commit `e3a37a7`）**
- backup `cards.db.before-psa-pop-backfill-20260526-003938`（815MB）
- 建 `psa_pop_refresh_log` 表（24h 限速追蹤、3 column + 複合 index）、寫進 init_db()
- 清掉舊 3,575 卡「拍賣筆數」資料（SET NULL）

**3. PSA scraper（commit `76f2e2b` / `923c969` / `026f38d`）**
- `PSASession.get_population_summary(spec_id)`：打 PSA POP API、過 Cloudflare、回完整 JSON dict
- `PSASession.search_spec_ids_with_titles(query)`：DOM query 回 `(spec_id, anchor_text)` 對映、用 innerText 解決 anchor 子元素 silent fail（reviewer 抓的 latent bug）

**4. backfill 腳本（local-only `_backfill_psa_pop.py`）**
- gating 用 expanded high-rarity list（22 個 rarity 字串）— 5,489 張 pending（spec 估算 7-9k 偏高）
- post-filter `_pick_matching_spec`：三層 filter（set_name token + card_number + 卡名 token）— 抓到 dry-run 跨 set 同名 M Lucario 誤對映、迫使加 set_name token 防線
- ORDER BY `name IS NULL` 排尾、避免 set 950 純日文卡阻塞前 30 分鐘
- 背景啟動 PID 66156、~5 小時 ETA

**5. 後端 API（commit `0f5806d` / `68762d8`）**
- `get_card_prices`（spec 寫成 `get_card_detail` 是筆誤）SQL 加 3 個 subquery：snkr_raw_avg_jpy（裸卡 SNKR 近 90 天 AVG）、psa10_market_jpy（PSA 10 近 90 天中位數、window function 寫法）、psa10_market_n（樣本筆數）；response dict 加 4 個欄位（含 psa10_market_source flag）
- 修兩處 spec 筆誤：`sold_at` → `sale_date`、OFFSET subquery aiosqlite 不支援 → ROW_NUMBER OVER 寫法
- `_maybe_lazy_refresh_pop`：30 天過期 + 24h 兩次限制、非阻塞 `asyncio.create_task`
- `_do_single_psa_pop_refresh` / `_sync_single_psa_pop_refresh`：async wrap sync PSASession、共用 backfill 同樣的 search + POP API 流程
- `POST /api/psa/pop/refresh/{set_id}/{card_number}` 手動 endpoint：套 24h 兩次限速、回 `{ok, updated_at}` 或 `{ok: false, reason: rate_limited}`
- spot-check：en-Diamond-Pearl/122 manual refresh 成功 pop10=58 / total=852 / gem_rate=6.81%
- 用 patch 法拆 commit（cp 備份混合 → checkout HEAD → 重做改動 → 還原）、保留前 session 既有 M app/main.py 未 commit 改動

**6. 前端 Dashboard（commit `..\卡波\` 72eeb56）**
- 刪舊「PSA 拍賣分布」49 行區塊
- 加 51 行 CSS（.psa-dashboard / .psa-rec-* / .psa-gem-circle / .psa-calc-table / .psa-pop-bar / .psa-disclaimer / .psa-stale-hint）
- 加 224 行 JS：PSA_FX 派生匯率（reuse 既有 hardcode 0.22 / 32.0）、PSA_TIERS 4 級 PSA Japan 鑑定費、PSA_SHIPPING_JPY = 800
- `buildPsaDashboard(c)`：5 元件 — 推薦結論 header + 圓圈通過率（conic-gradient）+ 試算表 5 row + POP 5 cell + 「※ 試算價格僅供參考」disclaimer
- `calcPsaRecommendation`：4 狀態（insufficient / recommended / marginal / not_recommended）、通過率 + 淨利雙標準（>80% AND >¥1,500 / ≥60% AND >-¥1,500 / 其他）
- 3 個 handler：`psaChangeCurrency` / `psaChangeTier` / `psaManualRefresh`、localStorage 記住、即時重 render
- 過老資料（> 30 天）顯示「資料 N 天前 · 點此更新」

#### 進行中

- **Task 5 backfill 在背景跑**（PID 66156、29.6 min runtime）：DB has-POP 285 / pending 5,249 / 進度 5.1%、ETA 重估 ~5 小時（老 EN set EX-Legend-Maker / Aquapolis 等 PSA APR mapping 缺、~50% unmatched）
- **Task 12-14 + 16 待辦**：24h 限速驗證 / lazy refresh 觸發驗證 / 資料缺失降級驗證 / user 實機驗收 Dashboard — 都需 backfill 跑完（避免 cards.db 並行 writer 撞鎖）

#### 踩到的坑（已加 Known Pitfalls）

7 條：

1. **card_list.rarity 用英文全名**（'Special Art Rare' / 'Ultra Rare'）、不是縮寫 SAR/UR — gating SQL 第一版錯
2. **PSA search 第一個結果不一定對應該卡** — Ascended Heroes 5 張不同卡寫進同一 spec_id（PSA APR 已收的泛 set hit）
3. **PSA APR mapping 對英文 set 用 master numbering**（#274+）、跟 DB 用 Pokémon-only #1-150 對不上 — 影響 Ascended Heroes 等新 EN set
4. **regex 解 HTML anchor 對 nested `<span>` silent fail** — 改 DOM query（page.evaluate + innerText）
5. **aiosqlite 不支援 OFFSET subquery 引用外層 alias** — 改 window function 中位數寫法
6. **spec 筆誤 column name `sold_at`、實際 `sale_date`** — PRAGMA 確認改正
7. **PSA APR `_pick_matching_spec` 跨 set 同名同號污染**（cn=113 誤命中 XY Furious Fist M Lucario）— 加 set_name token 第三層 filter 防

#### 明天的下一步

1. **Task 5 backfill 跑完**（~5 小時 ETA）後 check 覆蓋率 + 高稀有度 0-row 列表給 user verify
2. **Task 12-14 驗證**：跑 24h 兩次限制 / lazy refresh 30 天觸發 / 資料缺失降級
3. **Task 16 user 實機驗收**：給 user 帶 cache buster 的 URL hard reload、看 Dashboard 實機效果、收 feedback 微調
4. **延續其他未動方向**：SM-P eBay backfill 結果 / Day 2 起手 S-P 劍盾 promo / 71 張高稀有度 0-row eBay verify / Portfolio Phase 2 / MVP S1 auth KYC

### 2026-05-26（午後）— 今日熱門頁單卡縮圖大小不一致修補（D 方向 canvas 動態 bbox）

開工 /today 後 user 反映看到的問題、走「列問題 → 討論 → 修」流程。session 中段被 user 糾正兩次「改動前你都要問我」、調整成「動手前 final confirm SQL/diff」流程。最後縮圖一致性走 D 方向解決、user 驗收「有了 可以」。

#### 完成

**1. M5「擴充包『深淵之瞳』」(pg=954) era 對映 INSERT → DELETE 退訂**
- 發現：user 截圖看「深淵之瞳」卡盒在「Other」分組、應該在「MEGA」期
- 查證：`jp_set_era_map` 漏 pg=954 對映、同樣漏的還有 pg=9005（Sun & Moon promo）
- INSERT (954, 'M5', 'MEGA') 後 user 反彈「改動前都要問我」、改 DELETE 退訂、DB 回到 5/25 收工狀態
- backup `cards.db.before-era-map-fix-M5-20260526-012953` 仍保留

**2. SNKR 今日熱門頁單卡縮圖大小不一致修補（核心工作）**

問題本質：`snkr_hot_items` 上 21 張單卡縮圖、jp_card_list 來源（純卡圖 0.716 ratio）跟 SNKR cdn 來源（卡置中、padded 在 1.368 canvas）兩種源、視覺大小差很多。

迭代過程（8 輪）：
1. **cover 換 contain → contain 換 cover**：jp_card_list 卡填滿、SNKR cdn 仍小
2. **cover + scale(1.35) on SNKR cdn**：切到圖
3. **contain + scale(0.55) on jp_card_list**：太小
4. **scale(1.15) 中間值**：仍不夠一致
5. user：「用 brainstorming 解決」+「列出問題然後跟我討論」
6. **走 brainstorming skill**：列出問題（兩源不同、padding 變異、CORS 阻擋 canvas）+ 4 條方向（CSS hack / cover only / 動 backend + canvas / backfill）
7. user 選 **D：動 backend + canvas 動態偏移**
8. 實作完成、user 驗收 ✅

**D 方向實作細節**：
- **Backend** (`app/main.py:4257`)：`proxy_img` 白名單加 `cdn.snkrdunk.com`（+1 行）
- **Frontend** (`..\卡波\index.html`):
  - 加 `analyzeSnkrCdnImage(cdnUrl)` 函式：透過 `/api/proxy_img?url=...` 同源 fetch SNKR cdn 圖 → canvas → 找非白 pixel bbox → 回 `{heightPct, widthPct, centerXPct, centerYPct}` → cache 進 `localStorage['snkrCdnBbox']`
  - 加 `setupSnkrCdnTransforms(container)` 函式：對 container 內 `img[data-snkr-cdn]` 元素跑 async analyze、套 `transform: scale(1/heightPct) translateY(...)`、clamp [1.0, 1.4] 防極端值
  - 兩處 render（trending carousel + snkr_hot view）：
    - img wrapper 加 `overflow:hidden` + `aspect-ratio:5/7`
    - SNKR cdn img 加 `data-snkr-cdn="${imgSrc}"` attr
    - innerHTML 設完後 call `setupSnkrCdnTransforms(box)`
- **效能**：第一次 21 張卡 × ~50-100ms canvas 分析 ≈ 1-2 秒（async 不擋頁 render）；之後 localStorage 命中、零延遲

**3. backend 重啟驗證流程**
- 第一次 kill listener PID + Start-Process 新啟、playwright 仍 403 — debug 發現 watcher 接管了 OLD code（PID 啟動時間早於 main.py mtime）
- 第二次 kill PID + 確認新 PID 啟動時間 > main.py mtime + python urllib 真打 endpoint 驗 200 + CORS *、才往下走

#### 進行中

無 — 全部完成、user 驗收 ✅。

#### 踩到的坑（已同步寫進 Known Pitfalls）

1. **AskUserQuestion 選項 ≠ 動手批准** — user 兩次糾正「改動前都要問我」、即使 option description 寫明確 SQL 也要 final confirm
2. **HTA watcher 接管 OLD code PID** — 改 backend 後 watcher restart 跑舊版、netstat PID 看似新但啟動時間早於 main.py mtime
3. **CSS 純 object-fit 沒辦法處理「圖內含 padding 且 padding 量變異」** — 兩源資料只能走 D 方向 canvas 動態量 or backfill
4. **設計需求迭代 8+ 輪才換思路** — CLAUDE.md「超過 3 輪不對主動 stop」原則本次驗證、user 提示才換 brainstorming
5. **AskUserQuestion option description 寫亂碼** — 幾個 option 的 description 末尾跑出無意義字元（「股股股」「藏視覺一致」「冷冷冷」等）、proofread rule 再次違反

#### 明天的下一步

1. **commit 今天的 fixes**：(a) `app/main.py` proxy_img whitelist 改動 + 之前 PSA POP scraper 改動（共 95 lines diff）、(b) `..\卡波\index.html` D 方向 canvas 邏輯（+90 lines）+ 之前其他 5/26 累積改動。**注意**：兩個 repo 都有混合改動、commit 前用 `git diff --numstat` 拆開語意分開 commit
2. **D 方向觀察期**：跑一段時間看 localStorage cache 命中率、有沒個別卡 bbox 算錯、user 看一陣子有沒新發現的視覺問題
3. **延續未動方向**：
   - 5/26 凌晨延伸 PSA POP Task 16 user 實機驗收（cache buster URL）
   - 5/26 凌晨 SM-P eBay backfill 結果 check
   - Day 2 起手 S-P 劍盾 promo
   - 5/25 巡檢剩 B2/B4/B5/B6
   - 71 張高稀有度 0-row eBay verify
   - Portfolio Phase 2 / MVP S1 auth KYC / GoldenGem Phase A
...
### 2026-05-26（晚）— SNKR cdn 卡圖橫式畫布裁切 + 卡片詳情頁稀有度分階特效（4 階）

User 報「M5 #87 ヤドラン 卡圖大小跟網站設定不同」。診斷後做了 image 修法 + 順手新增詳情頁稀有度分階特效（4 階）。整 session 反覆視覺化 mockup → user 選 → iterate 的「邊做邊看」流程跑了 8+ 輪。

#### 完成

**1. SNKR cdn 卡圖橫式畫布裁切（backend commit `7d6f12b`）**
- 加 `/api/cropped_snkr_img` endpoint：fetch SNKR cdn webp + PIL `getbbox()` 找非透明 bounding box + crop + 強制升級到原圖（strip `?size=*` query、拿 xl 1000×730 而非 medium 縮圖、避免「拿縮圖 crop 後瀏覽器再放大」糊）+ 回 PNG + ETag + Cache-Control max-age=300
- `_compute_image_url_local` 對 SNKR cdn URL 自動 wrap 成 cropped endpoint + 加 `&v=hd1` cache buster（繞 browser cache、邏輯改 bump 版本）
- `get_card_prices` 對 `jp_card_list` fallback 路徑（line 2123）改用 `_compute_image_url_local` wrap、不再直接 return 原 SNKR URL
- 背景：SNKR `/upload_bg_removed/` 是 1000×730 橫式畫布、卡牌本體在中央 ~429×610 直式、左右 ~57% 透明 padding。直接 `<img>` 顯示塞 5:7 容器只占 ~50%、user 抱怨「卡圖太小」。修後 M5 #87 ヤドラン 容器填滿率 50% → 85%

**2. 詳情頁卡圖 holo 彩虹反光 CSS 刪除（frontend commit `a7598c2` 內含）**
- User 看到 hover 出現「奇怪彩色光暈」反映、grep 找到 `.card-tilt .holo` (`..\卡波\index.html` line 201-215) 用 conic-gradient + `mix-blend-mode: color-dodge` 疊在卡圖上、user 表「我們網站不是用這種特效」
- 刪整段 holo CSS、保留 `.card-tilt .shine` 白色斜光（user 確認要留）

**3. 卡片詳情頁稀有度分階特效 4 階（frontend commit `a7598c2`）**
- mockup iteration：`_card_fx_mockup.html`（10 變體）→ user 選金箔 → `_card_fx_tier_mockup.html`（4 階對照）→ `_t4_gacha_mockup.html`（T4 4 種抽卡感變體、user 選 C 金黃白漸層）
- production paintDetail JS 加 tier 計算：rarity → `.rarity-tN` class 加到 `.detail-img-box`、innerHTML 加 `.card-bob` wrapper + `.sparkle-layer × 12` + `.holo-under`
- 4 階：
  - T1 RR：金箔邊 + 3D 傾斜（純）
  - T2 AR / CHR：+ 12 顆金色 SVG 4 角星形閃爍 + 旋轉（idle 2 秒、hover 1.3 秒）
  - T3 SR / SSR / CSR / SAR：+ 卡邊 6 色彩虹脈動發光（5 秒一輪）+ 上下飄動（3.2 秒一輪）
  - T4 UR / HR / MUR / MA：+ 卡邊金黃白漸層脈動（白心 → 金外 3 層、2.2 秒一輪）+ 上下飄動
- DOM 加 `.card-bob` wrapper 套 `float-up-down` animation、跟 `.card-tilt` 的 JS 3D transform 不撞
- sparkle-layer / holo-under element 一直在 DOM、CSS 依 `.rarity-tN` 才觸發顯示（不用 JS 動態插刪）

**4. 兩個 commit**
- backend `7d6f12b` feat(api): SNKR thumb_url 自動裁切透明邊 + cropped_snkr_img endpoint
- frontend `a7598c2` feat(detail): 卡片詳情頁稀有度分階特效（4 階 RR / AR-CHR / SR-SAR / UR-MA）

#### 進行中

無 — 4 階特效已套 production、user 驗收 4 個 URL 都 OK。

#### 踩到的坑（已加進上方 Known Pitfalls）

7 條新加：

1. PIL `Image.size` 報的是畫布尺寸、不是內容尺寸；webp 含大量透明 padding 時要用 `getbbox()` 找非透明 bounding box 才能算實際內容 ratio
2. endpoint Cache-Control max-age 設長（例 86400 = 1 天）會撞 browser cache、邏輯改後 user reload 拿不到新版；修法：短 max-age + ETag + wrap URL 加 cache buster query
3. CSS animation 跟 JS set transform 同 element 撞（兩個都改 transform 屬性互蓋）；解：加 wrapper element 分層套不同動畫
4. multi-layer overlay 系列 element z-index 要 enforce 一致 ≥ 主元素（sparkle / holo-under 等漏設一個就被主元素遮住、user 看到「跑到底下」）
5. drop-shadow 多層太重會把小元素糊掉、看不到本體；對 sparkle 類小發光元素用 SVG path shape + 1 層 drop-shadow 比 div 圓 + ::before/::after spike + 3 層 drop-shadow 更清晰
6. MCP playwright chrome residue 卡住 `browser_navigate`、要找 mcp-chrome-XXX user-data-dir process 殺掉再 navigate（不只殺最外層、相關 sub-process 也要殺）
7. 寫 `AskUserQuestion` option 文字反覆出現錯字 / 詞義不清（「複製品」「實千寥隻」等）— 即使 memory 規則已寫多次仍違反、要 implementation 階段每字念過 proofread

#### 明天的下一步

1. **確認 4 階特效在實際 production 各種稀有度卡都對**：user 自己開不同稀有度卡的詳情頁看效果是否如預期、特別是 T2 sparkle 在不同卡圖背景上的視覺對比
2. **延伸：set 列表頁 / 卡片格（card-item）也加 4 階特效？** — 目前只 detail page 套、user 可能想 set 列表縮圖也依稀有度有對應提示。要看 user 意願
3. **延伸：EN / TW 卡表的 rarity 字段是否能對映到 4 階？** — `en_card_list.rarity` 用英文全名（如 "Special Illustration Rare"）、跟 `jp_card_list.rarity` 縮寫（"SAR"）不同、要寫 mapping table
4. **延續 5/25 收工列的未動方向**：B（小修補）/ C（中型功能補洞）/ D（盒裝交易 Phase C）/ E（LINE Bot）
5. **未 commit 的 working tree**：backend `CLAUDE.md` / `PROGRESS.md`（user 改動）、frontend `卡波.hta` deletion / `.vscode/` / `trainer_sprites/` — user 自己處理或等他指示一起 commit

### 2026-05-26（深夜 — 開工接續）— 5/25 巡檢 B 系列 4 條 bug 收尾 + 主 repo 截圖清理

/today 開工、user 選「5/25 巡檢剩 4 個小 bug」為主軸（B2 / B6 / B4 / B5）、過程逐條 verify 實際狀態、結論是 code 0 行改動、4 條都不需動。

#### 完成

**1. B2 fetch 沒帶 API_BASE — 驗證為「已修」**
- grep `..\卡波\index.html` 全檔 fetch / `/api/`、所有呼叫都走 `api()` helper（line 1771 內部接 API_BASE）、沒有漏帶 prefix 的直接 fetch
- 5/25 凌晨 backup `index.html.before-pixel-merge-20260525-015723` 也是同狀況、所以 B2 在 5/25 巡檢之前就已修、PROGRESS 沒同步更新

**2. B6 EN「Other」分組過大 — 驗證為「已修」**
- DB query：`en_card_list_set` 174 個 set、`series` 欄全部非 NULL、16 個唯一值全部對映到前端 `SERIES_ORDER` 內、無「Other」fallback
- API `/api/cardlist/sets?language=en` 實際回 response 同樣全對映
- 前端 `bySer['Other']` 為空陣列、不會 render Other 區塊

**3. B4 hero 切語言時「成交紀錄」不變 — user 決議「跳過」**
- playwright 實機切 jp / en 模式測 5 個 hero 元素、發現現狀反過來：h1 / p / 「收錄系列」/「卡片總數」label 全中文 hardcode、只第三個「成交紀錄 / Sales History / 取引履歴」是三語切換
- 給 user 4 個方向選（一致中文 / hero 全切 / 加副標 / 跳過）、user 選跳過、因為 CLAUDE.md「介面以中文為主」原則允許 hero 大部分中文 hardcode

**4. B5 portfolio demo 跨遊戲卡 — user 決議「跳過」**
- 找到 `_getPortfolioMock()`（line 6106）含 16 張 demo 卡：5 寶可夢 / 4 ONE PIECE / 4 遊戲王 / 3 MTG
- 給 user 3 選項（換成 11 張寶可夢 mock / 直接刪 11 張只留 5 張 / 跳過等 Portfolio Phase 2 真接通）、user 選跳過

**5. 主 repo 8 個 debug 截圖清掉**
- `_check_card87_original.webp` + `snkr_cdn_inspect.png` + `snkr_hot_*.png × 6`
- 都是 5/26 晚 SNKR cdn 卡圖裁切 + 4 階特效 debug 階段的截圖、production 不再需要
- 直接 `rm` 刪除、不進 repo

#### 進行中

無 — B 系列 4 條 bug 已收尾、主 repo 截圖清乾淨。剩待 commit：CLAUDE.md（+2 規則）+ PROGRESS.md（含本段）。

#### 踩到的坑

無新踩坑 — 整 session 都是 verify 既有狀態 / 拿 user 決策、沒寫 production code。

#### 明天的下一步

1. **延續未動方向**（5/26 收工列已有、本日未推進）：
   - Day 2 起手 S-P 劍盾特典卡（119 件、接 5/26 凌晨 SM-P 流程）
   - PSA POP 推薦器 Task 12-14 + 16 user 實機驗收（5/26 凌晨延伸）
   - SM-P eBay backfill 結果 check + 高稀有度 0-row verify（5/26 凌晨開的背景）
   - Portfolio Phase 2 真接通（5/22 spec、本日 user 決議跳過 B5 等這個一起解）
   - MVP S1 auth + KYC + 廠商工作台（5/19 plan、最久沒動）
2. **本 session 截圖清掉後**仍有：卡波 repo `卡波.hta` deletion + `.vscode/` + `trainer_sprites/` 314 個 PNG 待處理、本日未推進

### 2026-05-27（凌晨 — 開工接續）— 詳情頁卡圖改造 + 今日熱門漲跌幅 + 圖表資料表 + 閃光變體系統 Phase 1-7 全套上線

/today 開工選「修一些小東西」、結果開到變體系統全套（球閃 / 屬性閃 / 大師球閃 / 通用閃 / 火箭隊閃 / SA 特別藝術 6 類）資料層 + 前端、外加詳情頁 / 今日熱門 / 圖表三個小修補。整 session 跨日 5/26 PM → 5/27 02:00+。

#### 完成

**1. 詳情頁卡圖修補（A+B 雙修 + 拿 pixel mode override + 全部特效刪、留翻面）**
- 卡圖白邊：`object-fit:cover` + 拿掉 .detail-img-box padding/background/border（line 215 / 377 / 4 個 responsive @media）
- pixel mode override（line 879）整條刪：拿掉黑邊 + 白底 + 像素陰影 + 直角
- 5/26 晚加的稀有度 4 階特效全刪（CSS 124 行 + JS innerHTML 簡化）、留翻面結構（card-faces）
- 流程：4 個 mockup 並排 → user 選方案 → 套 production → iterate

**2. 今日熱門 carousel 漲跌幅（後端 `/api/snkr/hot` + 前端 carousel）**
- 後端：找「至少 12h 前」最近 batch、lookup price_jpy mapping、加 `price_jpy_yesterday` / `price_change_pct` / `yesterday_fetched_at` 三欄
- 前端：價格區塊用 `margin-top:auto` 推到底（解卡名長短不齊導致價格高低不齊）、加 `.rc-change` 三色（紅漲 / 綠跌 / 灰持平、台股慣例）
- 持平也顯「→ 0%」灰色（user 反饋「為什麼沒顯示」、explicit 加進去）
- disclaimer 區擴充：加「漲跌幅對比 X（約 N 小時前）」

**3. 圖表加「完整成交紀錄」摺疊表格（line 364 CSS + paintChart 內 renderChartDataTable 函式）**
- 表格依當前時間範圍同步（7d / 30d / 90d / 1y / 全部）
- 5 欄：日期 / 價格 JPY / 價格 NT$ / 來源（含色標）/ 連結（外連 SNKR listing）
- 預設折疊、user 點「展開完整成交紀錄 (N 筆)」才展、max-height 400px sticky header

**4. 閃光變體系統 Phase 1-7 全套上線**

User 給兩條 ebay listing URL 點出問題：「同一張 M2a #127 Rayquaza 有 standard 跟 Reverse Holo Dragon 兩種、價差 NT$2,528 vs NT$4,971、現在 chart 混算趨勢失真」。Audit 確認 DB 99.2% 沒記變體、走「升級爬蟲」根本解路線。

- **Phase 1** 探勘：snkrdunk_mapping 232k row 內 8,233 含 `:` 變體 marker（3.5%）、50 種 unique marker、user 選範圍 B = 5 種閃法 + SA 特別藝術
- **Phase 2** `snkrdunk_mapping` 加 variant + variant_marker 欄、寫 `_parse_variant.py` 7 類分類 helper（self-test 22/22 過）、**2,923 row UPDATE 完成**
- **Phase 3** `card_prices` 加 variant 欄、寫 `_parse_variant_title.py` 英文 listing_title parser（含 Poke Ball #64 item 卡 FP 防護、self-test 18/18 過）、SNKR 用 apparel_id JOIN mapping + ebay/tcg/pc 用 listing_title regex、**82,825 row UPDATE 完成**（standard 86 萬 / SA 75,548 / mirror 5,852 / masterball 1,103 / pokeball 322）
- **Phase 4** `get_cards_by_set` 改 jp branch 加 variant 擴張：對每張卡 query mapping 找 non-standard variant、複製 row 加 variant 欄。M2a (set=950) 從 197 → 486 卡（**1.43x 擴張**、跟 Phase 1 估算 1.4x 完全吻合）
- **Phase 5** `/api/prices/{set_id}/{cn}` 接 `?variant=` 篩選、SQL 動態加 variant filter、SELECT 多取 variant 欄、response 加 variant echo
- **Phase 6** 前端 7 處改動：CSS 加 `.v-label` 7 色配色、`VARIANT_LABEL` 中文對映 const、navigate / parseHash / goDetail 加 variant 參數、cardItemHtml 每變體 1 個 card-item + 變體小標籤、renderDetail call API 帶 variant、paintDetail 詳情頁 d-meta 顯變體 pill、state.variant 跨頁殘留防護（parseHash 顯式 null reset）
- **Phase 7** 變體真實卡圖批量爬：snkrdunk_mapping 加 image_url 欄、寫 `_backfill_variant_images.py` httpx async 5 並行抓 SNKR 商品頁 og:image、**2923/2923 全成功、45 秒跑完**、endpoint expand 加 image_url override（變體用 SNKR cdn 圖、缺則 fallback standard）

驗證 M2a #127 完美：standard 用 jp_card_list 一般版圖 / pokeball 用 SNKR cdn 球閃變體圖 / type_pattern 用 SNKR cdn 屬性閃變體圖、跟 user 給的 ebay listing 對映 100% 對。

#### 進行中

無 — Phase 1-7 全完成、user verify pending（剛上線、user 還沒實機看）。

#### 踩到的坑（已加上方 Known Pitfalls）

5 條新加：

1. AskUserQuestion proofread hook 過於 aggressive、卡循環 — 撞到 2 次反彈就直接 fallback 文字列選項
2. task wrapper 報 exit 127 ≠ python process 真死 — 反覆撞 5+ 次、verify 真實狀態用 netstat / curl
3. 「Poke Ball #64」item 卡 FP — 球閃 regex 必須加 `reverse holo / mirror` context 才算變體
4. state.variant 跨頁殘留 — parseHash 結尾顯式設 null
5. httpx http2=True 要額外裝 h2 package — embedded Python 沒裝、預設 http/1.1 即可

#### 明天的下一步

1. **user 實機 verify Phase 1-7**：開 M2a 卡盒列表看 486 卡是否正確展開、#127 三張卡圖（standard / 球閃 / 屬性閃）是否正常、詳情頁 URL 帶 variant 是否正確、價格圖表是否依 variant 篩選
2. **後續可能要做**（順序未定）：
   - SNKR cdn 變體卡圖跟 standard 純卡圖視覺大小不齊（Pitfall #109 D 方向修法套到卡盒列表 card-item、目前只首頁 carousel 套）
   - EN / TW set 變體擴張（目前只 JP）
   - 拆 commit：本 session 累積 app/database.py / app/main.py / 卡波/index.html / cards.db 改動量大、要按語意拆開
3. **延續 5/26 未動方向**：Day 2 起手 S-P 劍盾特典卡 / PSA POP user 實機驗收 Task 12-14 + 16 / SM-P eBay backfill 結果 check / Portfolio Phase 2 / MVP S1 auth + KYC
