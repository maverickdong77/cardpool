...
DB = 'cards.db'
LOG = '_jp_rarity_backfill_v2.log'
...
def parse_rarity(html):
    # case-insensitive、抓 X / X_c 兩種變體
    m = re.search(r'/assets/images/card/rarity/ic_rare_([a-zA-Z]+?)(?:_c)?\.gif', html)
    if m:
        token = m.group(1).lower()
        return RARITY_MAP.get(token, token.upper())
    return None
...
def main():
    ...
    f.write("\n=== run start ===\n")
    f.close()
    log('JP rarity backfill v2 start')
    ...
    log('jp rarity final: ' + str(final) + ' / 20557')
