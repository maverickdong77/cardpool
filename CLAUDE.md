...
- **AskUserQuestion 選項 ≠ 動手批准、執行前要 final confirm**（2026-05-26 升級）：user 透過 AskUserQuestion 選一個 option（即使該 option description 內寫明確 SQL / Edit diff / 動作內容）**不算動手批准**。必須**在實際執行前再給 user 看一次最終指令、拿到 explicit「go」訊號**才能執行。流程改成兩段：(1) AskUserQuestion 拿方向 → (2) 顯示完整實際指令（SQL / patch / Edit diff / 將動的檔案）+ 拿執行批准 → (3) 才執行。**Why**：5/26 user 兩次明訓「我說過改動前你都要問我」、option description 寫的 SQL user 可能沒詳讀、選 label 後仍可能 reconsider。**How to apply**：凡是寫進 DB / 改檔案 / 動 schema / 跑 backfill 等**會留痕的動作**都套這流程。純查詢（SELECT / Read / Grep）不用、那些不留痕。
...
- **設計類決策 user 不確定時、預設一次生 4+ candidate 並排 mockup、不要先做 1 個再 iterate**（2026-05-26 user explicit feedback 升級、強化既有「設計類決策優先生 preview / mockup」原則）：user 反覆強調「不確定的話一樣視覺化給我選」+「多幾個選擇我才不用多問你好幾次」。觸發情境：對「特效 / 動畫 / 配色 / 排版 / 字體 / 圖形元素」這類視覺決策、user 描述模糊（如「要漂亮一點」「要抽卡的那種感覺」）或我自己對方向沒高把握時、不要先做 1 個給 user iterate、改成**一次生 4-10 個 candidate 並排 mockup**（含 idle / hover 行為都演示、附文字 hint 解釋差別）。實證 2026-05-26 卡圖特效從「先做 1 個 user 改」改成「10 變體並排 → user 選 → tier 4 階對照 → user 選」、來回輪數從 8+ 輪降到 3 輪。**標準 workflow**：(a) 列 candidate 維度（顏色 / 強度 / 速度 / 風格類型）、(b) `_*_mockup.html` 並排 N 個 stage、(c) 給 user URL 自己 hover 試手感、(d) user 選編號 → 套 production。**例外**：user 已明確指定方向（如「就用 #2」）、或 candidate < 2 個獨立軸（如純調速度的「快/慢」二選一可不必 mockup）。**通則**：對「視覺感受」類問題、文字描述對 user 無感、預設 mockup 先行。
